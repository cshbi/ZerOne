...
4. **(도전) 이모지를 '스티커 파일'로** — `challenge_sticker_emoji.py`
   → 유니코드 대신 `stickers/` 폴더의 파일을 읽어 보여 주는 `StickerMessage` 완성.
   실제 카톡 스티커 방식('이름만 전송, 받는 쪽이 로컬 파일 조회')을 손으로 만들어 본다.
   전송(`to_wire`)은 그대로, 표시(`display`)만 바뀌는 다형성의 이점을 체감.
   미완성 상태로도 실행되며 파일 안 [기대 출력]으로 스스로 채점.
