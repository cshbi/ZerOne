...
def handle(conn, addr):
    ...
    while True:
        ...
        # ========== 여기서부터 클래스 ==========
        msg = Message.parse_from_client(line)
        ...
        error = msg.validate()
        if error:
            print(f"[서버] ({msg.type_tag} 거부) {nickname}: {error}")
            conn.sendall(f"TEXT|시스템|{error}\n".encode("utf-8"))
            continue
 
        print(f"[서버] ({msg.type_tag}) {nickname}")
        exclude = conn if msg.broadcast_exclude_sender else None
        broadcast(msg.to_broadcast_line(nickname), exclude=exclude)
        #======================================================
    with clients_lock:
