...
# │ ★★★ IP(바인딩 주소) 변경 포인트 — 서버 ★★★                      │
# └──────────────────────────────────────────────────────────────┘
# HOST = 이 서버가 '어느 주소로 들어오는 접속을 받을지'.
#   "0.0.0.0" = 이 PC 의 '모든 랜카드' 에서 받는다 (권장, 그대로 두세요).
#     · 나 혼자 테스트:      http://localhost:8000        (같은 PC)
#     · 다른 PC 에서 접속:   http://<이 PC 의 랜 IP>:8000  (예: 172.30.1.20)
#   특정 IP("172.30.1.20") 를 적으면 그 IP 로만 받는데, IP 가 바뀌면
#   (공유기/DHCP) 끊긴다. 0.0.0.0 은 IP 가 바뀌어도 되므로 더 안전하다.
#   ※ 학생에게 알려줄 '접속 IP' 는 여기 값이 아니라 ipconfig/hostname -I 로 확인.
HOST = "0.0.0.0"
PORT = 8765          # 포트를 바꾸려면 client/app.js 의 8765 도 같이 바꿔야 함
