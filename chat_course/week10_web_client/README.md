...
## 다른 PC 에서 접속하기 (여러 명이 같은 서버로)

지금까지는 `http://localhost` 로 **혼자** 테스트했습니다. **다른 PC(학생들)** 가 같은 서버에
붙으려면 아래 3가지만 하면 됩니다. **코드는 안 고칩니다** (서버 `HOST` 는 이미 `0.0.0.0`).

### ① 서버 PC 의 랜 IP 확인
서버(=`serve.py` 를 켠 PC)에서:
- Windows: `ipconfig` → `IPv4 주소` (예: `172.30.1.20`)
- 리눅스/맥: `hostname -I`

### ② 학생은 그 IP 로 접속
학생 브라우저에서 (localhost 아님!):
```
http://<서버 IP>:8000/rooms.html      예)  http://172.30.1.20:8000/rooms.html
```
> 클라이언트(`app.js`)가 **접속한 주소를 그대로 따라가** WebSocket 도 그 서버로 붙습니다.
> 그래서 학생은 아무 설정도 코드도 안 바꿉니다.

### ③ 방화벽 열기 (서버 PC)
서버 PC 방화벽에서 **8000 · 8765** 인바운드 허용.
- Windows: `serve.py` 첫 실행 때 뜨는 방화벽 팝업에서 **'개인 네트워크 허용'** 체크.

---

### ⚠️ 중요 — 다른 PC 에서는 브라우저 설정이 한 번 더 필요합니다 (crypto.subtle)

브라우저의 암호화(`crypto.subtle`)는 **`localhost` 또는 `https`** 에서만 열립니다.
그런데 학생은 `http://172.30.1.20`(랜 IP·http) 로 접속하므로 **암호화가 막혀 채팅이 안 됩니다.**
(서버 본인 PC 의 `http://localhost` 는 되는데, 다른 PC 의 랜 IP 는 안 되는 이유가 이것입니다.)

**해결 — 학생마다 Chrome 에서 이 주소를 '안전한 주소'로 등록 (1회):**
1. 주소창에 입력 → 열기:
   ```
   chrome://flags/#unsafely-treat-insecure-origin-as-secure
   ```
2. 입력칸에 **서버 주소**를 적는다:  `http://172.30.1.20:8000`  (← 각자 서버 IP 로)
3. 오른쪽 드롭다운을 **Enabled** 로.
4. 아래 **Relaunch(재시작)** 버튼으로 브라우저를 껐다 켠다.
5. `http://<서버 IP>:8000/codec_test.html` 로 **자가진단 '모두 통과'** 확인 → 그다음 채팅.

> 이건 **Chrome 전용**이고 다소 임시방편입니다. flag 없이도 되게 하려면, 결국 **배포는
> `wss://`(TLS)** 여야 합니다 — 아래 "왜 이 AES 는 진짜 보안이 아닌가" · "ws/wss 차이" 참고.
> (교육적으로도 좋은 연결고리입니다: "왜 굳이 이런 설정이 필요하지?" → "진짜 답은 wss".)
